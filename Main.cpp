...
int main() {
	...
	NetworkSim<RepeaterProtocol::Fusion<NGHZ, decltype(topology)>, decltype(topology)> sim(topology);
	...
	std::ofstream output;
	if (mpiRank == 0) {
		output.open("./out/results_L=" + std::to_string(L) + ",A=" + std::to_string(alice) + ",B=" + std::to_string(bob) + ".txt");
...
#pragma omp parallel for
		for (int i = start; i < end; i++) {
			...
			localSuccesses[effI] += sim(rand, *kit, params[effI].p, params[effI].q, alice, bob);
		}
