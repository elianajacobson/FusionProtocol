#ifndef FUSION_DECOHERE_IPP
#define FUSION_DECOHERE_IPP
...
namespace RepeaterProtocol {
	...
	typename Fusion_Decohere<N_GHZ, Topology>::FusionResult Fusion_Decohere<N_GHZ, Topology>::operator()(RNG && rand, const Topology & topology, size_t k, double p, double q, typename Topology::GraphType::vertex_descriptor alice, typename Topology::GraphType::vertex_descriptor bob, DecohereDistribution && decDist) {
		FusionResult result;
		...
		//external links
		{
			...
			for (std::tie(it, end) = edges(topology.graph()); it != end; it++) {
				...
				for (size_t i = 0; i < k; i++) {
					//on success
					if (dist(rand) < p) {
						size_t t1_1 = decDist(rand);
						size_t t1_2 = decDist(rand);
						if (k - (i + 1) <= t1_1) {
							...
							if (k - (i + 1) <= t1_2) {
								...
								//add an edge since both vertices passed the time cutoff condition
